...
// * Wikipédia
  ...
  $(document).ready(function(){
    ...
    $.ajax({
        type: "GET",
        url: "https://pt.wikipedia.org/w/api.php?action=parse&format=json&exchars=450&exlimit=10&exintro=&explaintext=&exsectionformat=plain&prop=text&section=0&page="+algo+"&redirects=&callback=?",
        ...
        success: function (data, textStatus, jqXHR) {
            ...
            var blurb = $('<div></div>').val(markup);
 
            // remove links as they will not work
            blurb.find('a').each(function() { $(this).replaceWith($(this).val()); });
