...
var AIauthor = "Victor Ribeiro" // * Autor da I.A.
...
function mileyOnLoad() {
  fbCaps();
  mileyIni();
  getHist();
  clock();
  console.log('* nome da IA: ' + AIname);
  console.log('* apelido da IA: ' + AInick);
  console.log('* versão da IA: ' + version);
  console.log('* autor da IA: ' + AIauthor);
  console.log("* idade da IA: " + AIidade(AInascimento, hoje) + " anos");
  $(".v").html("v"+version);

  onLoad();
  }
// Abrir/Fechar menu de configurações
function config() {d.getElementById('config').style.display = 'block'; autocom();};
function closeConfig() {d.getElementById('config').style.display = 'none';};
// Abrir/Fechar menu de configurações por voz
...
// Abrir/Fechar Log de conversação
...
if (bname.match("IEMobile")) {bname = "Internet Explorer no Windows Phone"};
...
var AInascimento = new Date(2014, 1, 2); // ano, mes, dia
var hoje = new Date(); // Hoje
function AIidade(AInascimento, hoje) {
    var diferencaAnos = hoje.getFullYear() - AInascimento.getFullYear();
    if ( new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate()) <
        new Date(hoje.getFullYear(), AInascimento.getMonth(), AInascimento.getDate()) )
     diferencaAnos--;
...
var audio = d.getElementById('voz');
audio.src = link
+ "?key=" + apikey
+ "&r=" + rate
+ "&hl=" + idioma
...
function initMp3Player(){
	audio.src = d.getElementById('voz').src;
