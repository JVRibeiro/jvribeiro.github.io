...
'<span class="title_Bigger">SOPH<span class="glossing-letter">i</span>A</span>'],scripts="brain-script js/sophia.brain.js config-script js/sophia.config.js brain-arr-script js/sophia.js".split(" "),s_1=document.createElement("script"),s_2=document.createElement("script"),s_3=document.createElement("script");s_1.id=scripts[0];s_1.src=scripts[1];setTimeout(function(){s_2.id=scripts[2];s_2.src=scripts[3];s_3.id=scripts[4];s_3.src=scripts[5]},1E3);document.body.appendChild(s_1);document.body.appendChild(s_2);
document.body.appendChild(s_3);
function appLoad(){$("#screen").removeClass("animated fadeIn");_isFirstBoot||(localStorage.introduction="yes",document.location.reload());"yes"===_isFirstBoot?introductionScreen():window.setTimeout(function(){avatar._animation("default");changeActualInput("#user-input");annyangStart();content.style.display="block";$("#system-status").show({effect:"drop",direction:"down"});$(loadScreen).fadeOut()},2E3);micPermission=void 0===micPermission&&"true"===_micPermission?!0:!1}
intScr_nameInput.addEventListener("input",function(){0>=intScr_nameInput.value.length||intScr_nameInput.value.match(/([,.;/<>:?"!@#$%\u00a8&*()_+=\u00b9\u00b2\u00b3\u00a3\u00a2\u00ac\u00a7~^\u00b4`\[\]{}]|[\d]|[\s])/gi)?(intScr_confirmButton.disabled=!0,notification.create("error","Caracteres n\u00e3o permitidos!","img/icons/ionicons/alert-circled.png",3E3)):intScr_confirmButton.disabled=!1});
...
$(".title_Small").addClass("title_SmallLeft");window.setTimeout(function(){$(".gloss").addClass("glossing-letter spacing-letter")},6E3);window.setTimeout(function(){bootstrap.title.text(intScr_titleMsg[4])},8E3);window.setTimeout(function(){permissSettings()},11E3)},2E3)},minLoadTime)):($(loadScreen).fadeOut(),intScr.style.display="block",nameFirstSetting(),window.setTimeout(function(){avatar._animation("jello");bootstrap.title.text(intScr_titleMsg[2]);annyangStart()},2E3))}
...
function nameFirstSetting(){actualScreen="name";bootstrap.title.text(intScr_titleMsg[2]);intScr_genderDiv.style.display="none";$(intScr_nameDiv).delay(3E3).show("slide");window.setTimeout(function(){intScr_nameInput.focus();changeActualInput("#introducing-screen-name-input")},4E3);$(intScr_confirmButtonDiv).show({effect:"drop",direction:"down"});$(intScr_confirmButton).attr("onclick","genderFirstSetting()");intScr_confirmButton.innerHTML='<span class="icon ion-ios-arrow-right"></span>';intScr_confirmButton.setAttribute("onclick",
"genderFirstSetting()");intScr_confirmButton.disabled=!0;ai.say("Agora preciso que me informe seu primeiro nome apenas.",nameInputCommands);listening||notification.create("error","O reconhecimento de voz n\u00e3o est\u00e1 funcionando!","",5E3)}
...
!0;bootstrap.title.text('<span class="title_Small">Voc\u00ea \u00e9 do sexo masculino ou feminino, '+user.name+"?</span>");window.setTimeout(function(){$("#introducing-screen-gender-div .cs-placeholder").click();$("div.cs-skin-slide").focus();intScr_confirmButton.disabled=!1},1E3);ai.say("Ok, "+user.name+". Agora me diga se voc\u00ea \u00e9 do sexo masculino ou feminino.",genderInputCommands)}
