      ...
      ai =
      {
        ...
        action      :     function ( command )
        {
          ...
          if ( command.match( orders ) )
          {
              ...
              if ( ai.learning.hasOwnProperty( actualOrder )
                && ai.learning[actualOrder].hasOwnProperty( actualObject ) )
              {
                ai.learning[actualOrder][actualObject]();
              }
              else
              if ( ai.learning.hasOwnProperty( actualOrder )
                && ai.learning[actualOrder].hasOwnProperty( '' ) )
              {
...
// Adiciona um zero ao início dos minutos e segundos menores que 10
  tmp_secStr = new String( sec );
  tmp_minStr = new String( min );
  tmp_hourStr = new String( hour );
  tmp_dayStr = new String( d );
...
// Decide se usa o plural ou não em "dia" e "mês".
  aiAgeYears === 1 ? aiAgeYears_str = aiYearsStr[0] : aiAgeYears_str = aiYearsStr[1];
  aiAgeMonths === 1 ? aiAgeMonths_str = aiMonthsStr[0] : aiAgeMonths_str = aiMonthsStr[1];
...
// Funções
  // Cálculo de idade
    ...
    function sophiaAge ()
    {
      if ( aiAgeYears >= 1  && aiAgeMonths > 0 )
      { // Se ela tiver 1 ano de idade e mais de um mês, a idade é contada em anos e meses
        aiFinalAge = aiAgeYears + ' ' + aiAgeYears_str
                   + ' e '
                   + aiAgeMonths + ' ' + aiAgeMonths_str;

        return aiFinalAge;
      }
      else
      if ( aiAgeYears >= 1 && aiAgeMonths === 0 )
      { // Se a idade dela for inteira, a idade é contada em anos apenas.
        aiFinalAge = aiAgeYears + ' ' + aiAgeYears_str;

        return aiFinalAge;
      }
      else
      { // Senão, é contada apenas em meses.
...
function patterns ()
{
  ...
  { // Inicia um loop que executa apenas uma vez cada elemento da array brain.
    ...
    { // testa a entrada do usuário contra o "regex" e, se corresponder, executa o próximo bloco de instruções - se não houver correspondência o loop vai continuar a carregar o próximo valor.
